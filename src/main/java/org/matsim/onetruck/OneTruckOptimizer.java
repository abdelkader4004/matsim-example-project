...
final class OneTruckOptimizer implements VrpOptimizer {
	...
	public void requestSubmitted(Request request) {
		...
		org.matsim.contrib.dvrp.examples.onetruck.OneTruckRequest req = (OneTruckRequest)request;
		...
		schedule.addTask(new OneTruckServeTask(OneTruckOptimizer.OneTruckTaskType.PICKUP, t1, t2, fromLink, req));

		VrpPathWithTravelData pathWithCustomer = VrpPaths.calcAndCreatePath(fromLink, toLink, t2, router, travelTime);
		schedule.addTask(new DefaultDriveTask(OneTruckOptimizer.OneTruckTaskType.LOADED_DRIVE, pathWithCustomer));

		double t3 = pathWithCustomer.getArrivalTime();
		double t4 = t3 + DELIVERY_DURATION;// 1 minute for the delivery
		schedule.addTask(new OneTruckServeTask(OneTruckOptimizer.OneTruckTaskType.DELIVERY, t3, t4, toLink, req));
