...
public final class MultiTrucksRequestCreator implements MobsimAfterSimStepListener {
    ...
    public MultiTrucksRequestCreator(@DvrpMode(TransportMode.truck) VrpOptimizer optimizer,
        ...
        for (int i = 0; i < 100; i++) {
            var links = network.getLinks().values().toArray();
            Link fromlink = (Link) links[new Random().nextInt(network.getLinks().size())];
            Link tolink = (Link) links[new Random().nextInt(network.getLinks().size())];
