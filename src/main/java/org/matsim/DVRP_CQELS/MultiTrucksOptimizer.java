...
final class MultiTrucksOptimizer implements VrpOptimizer {
    ...
    private int  nbTrips[]={0,0,0,0,0,0,0,0,0};
    ...
    DvrpVehicle chooseVehicle(){
        DvrpVehicle m= (DvrpVehicle)thisfleet.get(rand.nextInt(thisfleet.size()));
        nbTrips[m.getId().index()]++;
        System.out.println(m.getId()+" : nbOfTrips:"+nbTrips[m.getId().index()]);
        return m;
    }
    @Override
    public void requestSubmitted(Request request) {
        double currentTime = timer.getTimeOfDay();
        DvrpVehicle vehicle = chooseVehicle();
